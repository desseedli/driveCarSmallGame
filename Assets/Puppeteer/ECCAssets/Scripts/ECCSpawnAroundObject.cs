...
namespace EndlessCarChase
{
	...
	public class ECCSpawnAroundObject : MonoBehaviour
	{
        ...
        private List<(Type type, float weight)> configList = new List<(Type, float)>
        {
            (typeof(ECCOriginalAI), 30f),
            (typeof(ECCRandomPointInCircleAI), 70f)
        };
            ...
            for ( index = 0; index < spawnGroups.Length; index++ )
            {
                ...
                if ( spawnGroups[index].spawnObjects.Length > 0 )
                {
                    ...
                    else
                    {
                        ...
                        if (spawnGroups[index].spawnIndex > spawnGroups[index].spawnObjects.Length - 1)
                        {
                            spawnGroups[index].spawnIndex = 0;
                        }
        ...
        public void RandomCreateAI(Transform newSpawn, ECCCar eccCar)
        {
            ...
            {
                if (selectedType == typeof(ECCOriginalAI))
                {
                    ECCOriginalAI eccOriginalAI = new ECCOriginalAI(newSpawn.gameObject, eccCar);
                    eccCar.SetAIMode(eccOriginalAI);
                    //Debug.Log("Create ECCOriginalAI");
                }
                else if (selectedType == typeof(ECCRandomPointInCircleAI))
                {
                    ECCRandomPointInCircleAI eccRandomPointInCircleAI = new ECCRandomPointInCircleAI(newSpawn.gameObject, eccCar);
                    eccCar.SetAIMode(eccRandomPointInCircleAI);
                    //Debug.Log("Create ECCRandomPointInCircleAI");
                }
            }
        }
        ...
        public void Spawn( Transform[] spawnArray, int spawnIndex, Vector2 spawnGap )
        {
            ...
            ECCCar eccCar = newSpawn.GetComponent<ECCCar>();
            if(eccCar != null && gameController.playerObject != eccCar)
            {
                RandomCreateAI(newSpawn, eccCar);
            }

            // Spawn an Object at the target position
            if (spawnAroundObject)
            {
                newSpawn.position = spawnAroundObject.transform.position;// + new Vector3(Random.Range(-5, 5), 0, Random.Range(-5, 5));
            }

            // Rotate the object randomly, and then move it forward to a random distance from the spawn point
            newSpawn.eulerAngles = Vector3.up * UnityEngine.Random.Range(0, 360);
            newSpawn.Translate(Vector3.forward * UnityEngine.Random.Range(spawnGap.x, spawnGap.y), Space.Self);
            ...
            if (Physics.Raycast(newSpawn.position + Vector3.up * 5, -10 * Vector3.up, out hit, 100, gameController.groundLayer))
            {
